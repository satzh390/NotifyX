...
func (mockStore *MockSubscriberStore) Get(ctx context.Context, customerID, subscriberID string) (domain.Subscriber, error) {
	args := mockStore.Called(ctx, customerID, subscriberID)
	
	// Handle function return values
	if fn, ok := args.Get(0).(func(context.Context, string, string) domain.Subscriber); ok {
		return fn(ctx, customerID, subscriberID), args.Error(1)
	}
...
func (mockStore *MockSubscriberStore) Delete(ctx context.Context, customerID, subscriberID string) error {
	args := mockStore.Called(ctx, customerID, subscriberID)
	return args.Error(0)
}
