...
import (
	...
	"github.com/notifyx/worker-email/config"
...
func main() {
	configPath := os.Getenv("NOTIFYX_WORKER_EMAIL_CONFIG")
	if configPath == "" {
	...
	switch cfg.Email.Provider.Type {
	case "smtp":
		emailProvider, err = provider.NewSMTPProvider(provider.SMTPConfig{
			Host:     cfg.Email.Provider.SMTP.Host,
			Port:     cfg.Email.Provider.SMTP.Port,
			Username: cfg.Email.Provider.SMTP.Username,
			Password: cfg.Email.Provider.SMTP.Password,
			From:     cfg.Email.Provider.SMTP.From,
		})
		if err != nil {
			logger.Error("failed to create email provider", slog.String("error", err.Error()))
			os.Exit(1)
		}
	default:
		logger.Error("unsupported email provider type", slog.String("type", cfg.Email.Provider.Type))
		os.Exit(1)
	}
	...
	if err := emailWorker.Run(ctx); err != nil {
		logger.Error("Email worker stopped with error", slog.String("error", err.Error()))
		os.Exit(1)
	}

	logger.Info("Email worker exited cleanly")
}
