...
import (
	...
	"github.com/notifyx/worker-sms/config"
...
func main() {
	configPath := os.Getenv("NOTIFYX_WORKER_SMS_CONFIG")
	if configPath == "" {
	...
	var smsProvider provider.Provider
	switch cfg.SMS.Provider.Type {
	case "sns":
		smsProvider, err = provider.NewSNSProvider(provider.SNSConfig{
			Region:    cfg.SMS.Provider.SNS.Region,
			AccessKey: cfg.SMS.Provider.SNS.AccessKey,
			SecretKey: cfg.SMS.Provider.SNS.SecretKey,
		})
		if err != nil {
			logger.Error("failed to create SMS provider", slog.String("error", err.Error()))
			os.Exit(1)
		}
	default:
		logger.Error("unsupported SMS provider type", slog.String("type", cfg.SMS.Provider.Type))
		os.Exit(1)
	}
	...
	if err := smsWorker.Run(ctx); err != nil {
		logger.Error("SMS worker stopped with error", slog.String("error", err.Error()))
		os.Exit(1)
	}

	logger.Info("SMS worker exited cleanly")
}
